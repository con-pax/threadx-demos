...
volatile uint32_t counter_semaphore = 0 ;
volatile uint32_t counter_flag = 0 ;
...
void u54_4(void)
{
    ...
    /*------------------------------------------*/
    mss_config_clk_rst(MSS_PERIPH_GPIO2, 4, PERIPHERAL_ON) ;
    ...
    MSS_UART_polled_tx(p_uartmap_u54_1," \r\n>>>>Program unexpected termination", 100);
    while(1);
#endif
}
...
void    thread_gui_entry(ULONG thread_input);
...
CHAR    *pointer;
    ...
    tx_thread_create(&thread_gui, "thread gui", thread_gui_entry, 0,
...
uint8_t  gpio0_bit13_or_gpio2_bit13_plic_13_IRQHandler(void)
{
    ...
    MSS_GPIO_clear_irq(GPIO2_LO, MSS_GPIO_13);
    return 0 ;
}
